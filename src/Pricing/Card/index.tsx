import * as React from 'react';
import { LicenseType, PaypalEnvironmentType, RegistryLink } from '..';
import RecommendedSvg from '../../svgs/recommended.svg';
import Link from 'next/link';

export interface IPricingCardProps {
    currenty: string;
    paypalEnvironment: PaypalEnvironmentType;
    paypalBusinessCode: string;
    isRecommended: boolean;
    licenseType: LicenseType;
    licenseCaption: JSX.Element;
    originalPrice?: string;
    discountedPrice: string;
    description: JSX.Element;
    registryLink: RegistryLink;
    purchase?: JSX.Element;
    subscribeType: string;
}

export default class PricingCard extends React.Component<IPricingCardProps, any> {
    public render() {
        return (
            <div className="license">
                {!!this.props.isRecommended && <div className="recommended"><RecommendedSvg /></div>}
                <div className="type">{this.props.licenseCaption}</div>
                <div className="splitLine"></div>
                <div className="price">
                    <span className="currency">{this.props.currenty}</span>{!!this.props.originalPrice && <span className="originalPrice">{this.props.originalPrice}</span>}<span className="discountedPrice">{this.props.discountedPrice}</span><span className="period">/year</span>
                </div>
                <div className="description">
                    {this.props.description}
                </div>
                {!!(this.props.licenseType != LicenseType.Free) && <div className="paypal">
                    <form action={this.props.paypalEnvironment == PaypalEnvironmentType.Sandbox ? "https://www.sandbox.paypal.com/cgi-bin/webscr" : "https://www.paypal.com/cgi-bin/webscr"} method="post" target="_target">
                        <input type="hidden" name="cmd" value="_s-xclick" />
                        <input type="hidden" name="hosted_button_id" value={this.props.paypalBusinessCode} />
                        <input type="hidden" name="on0" value="Subscribe" />
                        <input type="hidden" name="os0" value={this.props.subscribeType} />
                        <button className="button" type="submit" data-v-11a15d7f="" aria-label="PayPal" name="submit">{this.props.purchase}</button>
                        <span className="text">Powered by&nbsp;</span>
                        <img src="data:image/svg+xml;base64," alt="" aria-label="PayPal" className="logo"></img>
                    </form>
                </div>}
                {!!(this.props.licenseType == LicenseType.Free) && <Link href={this.props.registryLink.path}><a>{this.props.registryLink.caption}</a></Link>}
            </div>
        );
    }
}